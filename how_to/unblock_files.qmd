...
OubreakTools linelists rely on VBA macros for most of their functionalities. Windows often blocks files with macro when they come from the web (*i.e.* received through mail, or Teams, or from a `zip` archive as we just did) [You can read the [full explanation](https://learn.microsoft.com/en-us/microsoft-365-apps/security/internet-macros-blocked) of why and when this happens to better predict when you can expect it.]{.aside} so extra steps might are often necessary to access the file full functionalities.

::: {.callout-note}
This protection is independant from the password protection that a user or a developper can add to an Excel file.
:::


## Unblock files {#sec-unblock-file}

When you receive a file, before opening it, always check whether Windows blocked it or not, and unbloc it if needed:
...
![Trust Center menu for macros](../how_to/images/003-img-trust-center.png){fig-align="center" fig-alt="A screeshot of the trust center popup windows, on the Macro settings tab"}
    ...
    d) Tick the "Disable all macros with notification", or the "Enable all macros".
...
::: {.callout-warning}
If you have a red band informing you that macros have been disabled, but no button to authorise them, it means that the Trust Center option is to block them. Go check the Trust Ceenter options, as in step 3.
