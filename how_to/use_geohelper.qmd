...
To use it, place your cursor in a `geo` or `hf` column (you can recognise them to to their orange headers), in the *admin 1 level*. Click on the "GeoHelper" button in the [OBT menu](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html#sec-geo-helper), or hit the shortcut `Ctrl + Shift + G`. It opens a window with several tabs, depending on whether you are in a `geo` (described by four visible columns) or in `hf` (described by one visible column).
