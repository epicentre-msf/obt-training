...
Most of the important features of the linelist can be accessed through the [OutbreakTools Menu](https://epicentre-msf.github.io/outbreak-tools/reference/home_reference.html) at the top of Excel interface. 
...
OBT linelists usually exist in several languages. In that case there is one file per language (by opposition to a button to switch language withn one linelist).
...
All the OBT linelist produced by the EpiDS team have a button to [export data to the dashboard]. This creates a protected file with the data and metadata, that you can send to a dedicated mail address (see (@fig-workflow)).

More explanations can be found [here](../how_to/export_data_dashboard.qmd)
