...
Dans ce tutoriel vous apprendrez à connaître l'interface des listes linéaires OutbreakTools^[Pour rappel, toutes les listes linéaires OBT ont la même interface] et le tableau de bord correspondant.
...
- Accéder et savoir utiliser le tableau de bord intersection
...
Vous visiterez ensuite le tableau de bord Intersection pour explorer les données _réelles_ de MSF sur la rougeole au Tchad.

Les explications seront données dans le texte ou dans des encadrés (conseils, notes, etc.) et les instructions seront données dans des encadrés.
...
::: {.instructions}
1. Cliquez sur le bouton ci-dessous pour télécharger l'archive. Enregistrez-la sur votre ordinateur (dans vos *Téléchargements* par exemple)
...
downloadthis::download_link(
  ...
  button_label = "Télécharder l'archive",
  has_icon = TRUE,
  icon = "fa fa-save",
...
2. Dézippez l'archive pour sauver les fichiers localement. Il est conseillé de dézipper vers un emplacement qui n'est pas activement synchronisé avec OneDrive en ce moment.
:::
...
## Partie 0 : débloquer les fichiers et les macros {#sec-unblock-file}

Les listes linéaires OBT utilisent des macros VBA pour ajouter des fonctionnalités. Cela fait qu'elles doivent souvent être déloquées sur votre ordi lorsque qu'elles viennent d'internet, et vous devrez autoriser l'exécution des macros.


::: {.instructions}
Débloquez les fichiers ou assurez-vous qu'ils le sont déjà. Si vous ne l'avez jamais fait auparavant, [suivez ces étapes](../how_to/unblock_files.qmd)
:: :

:: : {.callout-note}
En fonction du niveau de protection de votre ordinateur et de la source des fichiers, vous arez peut être à le faire ou non. Par contre, les ordinateurs terrain sont souvent plus protégés et et est très probable que certain de vos collègues aient à le faire, donc soyez prêt à expliquer.
:::

## Partie 1: préparer et déployer une liste linéaire

### Obtenir fichiers nécéssaires

#### Un modèle de liste linéaire

Dans la vraie vie il vous faudra un [modèle vide](https://epicentre-msf.github.io/outbreak-tools/getting_started/get_linelist.html) pour vous lancer, mais dans le cadre ce cet exercice vous utiliserez le fichier vide inclus dans l'archive que vous avez téléchargée.
...
::: {.instructions}
Rendez-vous sur le site [GeoApp](https://reports.msf.net/secure/app/outbreak-tools-geoapp) et téléchargez une base géo pour le Tchad. Vous pouvez lire un résumé des étapes [ici](https://epicentre-msf.github.io/outbreak-tools/how_to/import_geobase.html#sec-geobase-download), ou directement sur le site, à l'ouverture.
:::

Le fichier n'est pas protégé, vous pouvez l'ouvrir si vous êtes curieux.


### Adapter une liste linéaire {#sec-customise}

::: {.instructions}
Ouvrez la liste linéaire et inspectez les (noms des) onglets
:::

**Notablement**: 

- La première feuille contient des instructions, il n'y a rien à faire dessus à part les lire
- La deuxième feuille ("Info") contient des informations sur le projet et le site (métadonnées)
- La troisième feuille ("Linelist patients") contient les données des patients
...
- `Nom du pays`: Tchad
- `Nom de la structure`: Moïssala Hospital
:::
...
![Fenêtre pop-up](../module_1/images/003-img-obt-show_hide.png){fig-align="center"}
...
::: {.instructions} 
Cliquez sur le menu "Importer une géobase" dans le [menu OBT](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html) pour importer la base géo que vous avez téléchargé précédemment.
:::

Après avoir reçus le message de confirmation, vous pouvez vérifier l'import en allant sur la feuille des données patients, et en regardant les colonnes "géo", celles avec un fond couleur saumon. Si tout s'est bien passé :
...
Vous pouvez remplir les colonnes géographiques soit à partir des menus déroulants en cascade (en commençant pas le niveau 1 !), soit en utilisant le bouton [GeoHelper](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html#sec-geo-helper) du menu OBT.
...
On peut aussi cacher ou montrer les variables optionnelles sur cette feuille (comme pour la feuille "Info"). Lorsque vous préparerez une liste linéaire, il vous faudra décider quelles variables collecter en fonction informations dont vous avez besoin^[Vous pouvez cacher ou montrer des variables plus tard, pendant la collecte des données si besoin, mais dans ce cas gardez en tête que les données pour ces colonnes ne seront que partielles.].


::: {.instructions} 
Placez(vous sur la feuille "Linelist Patients" et cliquez sur le bouton "Afficher/Masquer des variables". Cachez toutes les variables optionnelles à l'aide du bouton, et dé-cachez manuellement :
...
1. Une fois sur la nouvelle feuille créez, cliquez sur le bon vieux bouton "Afficher/masquer les variables" pour ouvrir la fenêtre pop-up pour le registre.

2. Cliquez que le bouton "Correspondre au Show/Hide de la linelist" pour aligner la visibilité des variables du registre sur celles de la feuille patients.

3. Cachez les variables qui n'ont pas besoin d'être dans le registre.
:::
...
- Ajouter des lignes à la table pour qu'elle fasse une page^[Astuce: il y a un bouton pour ça dans le menu OBT !].

::: {.instructions}
Ajustez la largeur des colonnes : 

- `Nom / Prénom`: plus large
- `Sexe`: moins large

Pour voir, cliquez sur le bouton "Imprimer" (toujours dans le [menu OBT menu](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html)). Remarquez le bouton "Mise en page" en haut à gauche, qui contrôle les paramètres d'impression.
...
Avec la même idée que pour le registre il est possible d'imprimer un formulaire de cas à partir de la liste linéaire s'il n'y en a pas déjà un dans la structure. Un bouton affiche une feuille préparée automatiquement. Créer un CRF propre est au delà de nos capacités pour le moment, mais ce brouillon devrait être une base et vous faire gagner du temps.
...
- Raccourcir des labels^[Désormais, une option a été rajoutée au ficher **setup** pour donner des labels courts ou des acronymes, mais ils n'ont pas été remplis pour toutes les liste linéaires OBT en circulation pour le moment.].
...
- Comment [débloquer le fichier et autoriser les macros](../how_to/unblock_files.qmd)
...
- Comment [exporter les données vers le tableau de bord](https://epicentre-msf.github.io/outbreak-tools/how_to/send_data_to_dashboard.html)
...
## Partie 2 : utiliser une liste linéaire au jour le jour

### 🧭 Contexte

La linelist a été déployée en Janvier 2022 à Moissala et les zones alentour. Un peu plus d'un an plus tard, en avril 2023 l'épidémie est malheureusement  toujours en cours, et la linelist toujours utilisée. Vous recevez le fichier avec les données régulièrement pour le suivi de l'épidémie...

Nous allons dans cette partie travailler sur le fichier `measles_linelist_moissala.xlsb` qui contient des données rougeole simulées^[En essayant au maximum d'utiliser les paramètres des linelists rougeoles de MSF, au moins pour les variables principales, ce qui nous évite des problèmes de confidentialité.].
...
Lorsque vous recevez une liste linéaire avec des données, nous vous conseillons d'effectuer quelques vérifications basiques en routine pour avoir une idée de l'état du fichier et des données. Ce ne sont pas les vérifications longues, complexes ou complètes que vous effectueriez si vous étiez responsables du suivi qualité des données, ou si vous vouliez effectuer des analyses. Mais ces vérifications vont vous aider à  détecter les sources de quelques problèmes plus rapidement.
...
::: {.instructions}
Effectuez ces vérifications sur le fichier `measles_linelist_moissala.xlsb`.
:::


#### Contrôle (très basique) des données

- Est-ce que les métadonnées sur la feuille "Info" sont correctement remplies^[90% des problèmes liés à l'export vers le tableau de bord sont liés à ça.]?
...
- Vérifiez quelques colonnes contenant des catégories (`sexe`, `sortie`...) pour voir si les catégories sont rentrée correctement
- Enlevez tous les filtres présents sur les données (il y a un bouton raccourcis pour ça dans le menu OBT)
...
1. Les analyses doivent être **mises à jour** manuellement avant utilisation.

![Bouton de mise à jour des analyses dans le menu OBT](../module_1/images/004-refresh.png){fig-align="center"}

2. Si vous filtrez les données dans la feuille patients, cela sera reflété dans les analyses *après mise à jour* (un texte en rouge vous avertira que c'est le cas).

3. Vous pouvez modifier l'apparence des graphiques (qui sont crées automatiquement et, nous le concédons, sont un peu moches). Vous pouvez pour cela utiliser les [outils classiques d'Excel](https://support.microsoft.com/fr-fr/office/mettre-en-forme-les-%C3%A9l%C3%A9ments-d-un-graphique-b6c787d5-f90a-41d2-a901-9d3ed9f0dbf0), comme vous le feriez pour vos propres figures^[Malheureusement, les modifications des graphiques ne peuvent pas être migrées vers une autre linelist, donc je vous conseille de vous restreindre aux graphes qui sont peu lisibles, ou que vous exportez régulièrement dans des rapports ou présentations, ou qui vous font _vraiment_ saigner les yeux.].

Nous allons commencer par explorer les analyses sur les données non filtrées.

::: {.instructions}
Placez-vous sur la feuille contenant les données patients, tapez sur la touche **F9** (force le calcul de toutes les formules), puis ouvrez le menu OBT et cliquez sur le bouton pour nettoyer tous les filtres. Ensuite, mettez à jour les analyses.
:::
...
- Combien des patients MAS sont positifs pour la test palu ?
:::

Intéressons nous à présent aux caractéristiques des patients décédés.

::: {.instructions}
1. Retournez dans la feuille des données patients. utilisez les filtres pour sélectionner seulement les patients décédés (dans le centre ou dans la communauté, comme vous voulez).

2. Rafraîchissez les analyses et retournez sur la feuille des tableaux simples.
...
Maintenant allons sur le feuille "Analyses temporelles". Sur cette feuille, vous pouvez choisir l'échelle temporelle à explorer : jours, semaines, mois etc. La table contient 53 lignes, et peut donc montrer : 
...
![Paramétrer les analyses temporelles](../module_1/images/007-img-temporal-table.png){fig-align="center"}
...
- Dans un premier temps, regardons les tables en unité "semaine". Est-ce que toutes les données sont représentées dans la table ?
- Combien de patients ont été déchargés lors de la semaine 18 de 2022 ?
...
- Changez les unités en "Mois". Combien de patients ont été admis en Janvier 2023 ?
:::
...
- utiliser le bouton pour filtrer (icone en forme d'entonnoir, le dernier) pour filtrer les catégories que vous ne voulez pas voir
...
![Graphe un peu amélioré pour publication](../module_1/images/009-img-temporal-hospit_pimped.png){fig-align="center"}


#### Lieu : analyses spatiales

Rendons-nous maintenant sur la troisième  feuille d'analyses. Elle nous montrer le **top 30** des zones de résidence des cas, et en dessous, le top 30 des zones d'origine des cas (pour les patients qui ne font pas partie de la population d'accueil)^[Le nombre 30 est défini dans le fichier **setup** et pourrait être différent dans une autre liste linéaire].
...
Pour cette linelist, les données intéressantes sont au niveau "zone de responsabilité" (niveau admin 3), mais en fonction de vos données, d'autres niveaux peuvent être pertinents à regarder, si les personnes voyagent beaucoup, ou si vous avez crée une liste linéaire "master" à partir de plusieurs fichiers.
...
2. Placez votre curseur sur une des cellules bleu clair à gauche (commençant à la cellule `C14`)

3. Cliquez sur le bouton ["GeoHelper"](../how_to/use_geohelper.qmd) du menu OBT et sélectionnez Moissala Est (niveau admin 3)

4. Allez à la cellule bleu suivante, et répétez les étapes pour sélectionner un autre endroit d'intérêt (vous pouvez regarder le top 30 de la feuille d'analyse spatiale pour identifiez des lieux d'intérêt, ou vous intéresser à un lieu avec des cas récents)
:::


![Paramètres des analyses spatio-temporelles](../module_1/images/010-img-spatio-temporal.png){fig-align="center"}
...
Vous avez déployé la liste linéaire sur le terrain. Une personne chargé de l'entrée des données débute sur le fichier OBT et a des questions... 
...
Les macros ne s'exécutent pas ou pas correctement. Le plus probable est que les macros ne sont pas autorisées; plus rarement, le projet VBA ne s'est pas lancé correctement ou a planté.

Demandez leur de fermer et réouvrir le fichier pour voir si ça résout le problème (en particulier si la linelist marchait chez eux avant). Si le problème persiste, vérifiez que le fichier est bien débloqué, et qu'ils ont bien autorisé les macros (voir @sec-unblock-file).
:::

### Méconnaissance générale du fichier

De nombreuses questions que vous pourrez avoir vient du fait que les gens ne lisent pas la documentation, y compris quand elle a été explicitement envoyée pour résoudre un problème. En particulier, les gens ignorent souvent l'existence du [menu OBT](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html). la bonne nouvelle est que un grand nombre de requêtes sont faciles à régler. 
...
**Note**: les versions récentes de la liste linéaire ajoutent automatiquement cinq nouvelles lignes lorsque l'on rentre des données dans la dernière ligne, donc ce problème devrait devenir moins courant. Par contre, ça veut aussi dire qu'il faudrait migrer les données vers une version plus récente de la linelist !
:::
...
> Bonsoir, nous avons entré des données dans la base de donnée mais des lignes manquent alors que nous ne les avons pas supprimées.
...
Il faudra discuter avec votre collègue pour essayer de comprendre ce qui a pu se passer et voir si une version peut être récupérer quelque part. mais commencez par vérifier les filtres au cas où ça soit un problème facile à régler. Quel bouton utiliseriez-vous ?

::: {.callout-tip title="Réponse"  collapse=true}
Le bouton "Supprimer tous les filtrer" du menu OBT.
:::
...
Une mise à jour récente^[Le problème est apparu sur certains ordis à partir de Novembre 2024.] d'Office 365 barre le texte qu'Excel considère comme "obsolètes" (une valeur qui dépend d'autres valeurs, qui elles ont été modifiées). Pour des raisons que nous n'expliquerons pas ici, le bouton "Actualiser les analyses" ne suffit pas (encore) à envoyer le signal à Excel que toutes les cellules ont été recalculées. Le problème va être réglé, mais en attendant, si ça vous arrive : 

- Sur la feuille patient, tapez **F9** (raccourcis pour le bouton "Calculer maintenant" du menu Excel "Formules").
- *Ensuite*, utilisez le bouton "Actualiser les analyses" du menu OBT.

Si le problème vous gène souvent (et sur d'autres fichiers que les listes linéaires OBT) il est possible de [désactiver](](https://support.microsoft.com/fr-fr/office/mise-en-forme-des-valeurs-obsol%C3%A8tes-4b5c63c3-5dc7-4e9b-8e24-88bed1987dbd) ) cette nouvelle fonctionalité dans Excel


### Envoi des données vers le tableau de bord

Une autre source commune de problèmes est l'étape du partage des données de la liste linéaire pour qu'elles soient mises sur le tableau de bord. Il est important d'expliquer les [étapes](https://epicentre-msf.github.io/outbreak-tools/how_to/send_data_to_dashboard.html) aux collègues qui devront effectuer la manip. 

Pour vous entraîner, voici quelques problèmes communs; pourquoi ça ne marche pas ? 


- Les données ont été partagées avec le compte <EPI-DATA> via OneDrive, et n'apparaissent pas sur le dashboard.

- Le fichier `measles_linelist_moissala.xlsb`a été envoyé par mail à <EPI-DATA@epicentre.msf.org> et une erreur est revenue.

Le fichier `measles_linelist_moissala_export_data_20250203-1136.xlsx` a été envoyé par mail à <EPI-DATA@epicentre.msf.org> et une erreur est revenue.

Le fichier `MSL_linelist__MSL24__info_country_code__info_organisation__info_facility_name__anonymous__vb-1003__20250103-1541` a été envoyé par mail à <EPI-DATA@epicentre.msf.org>. Il n'y a pas eu d'erreur, mais les données n'apparaissent pas sur le tableau de bord pour le Tchad...
...
Soit le fichier de la liste linéaire entier ou le mauvais export (export pour la migration) ont été envoyés, soit le bon fichier a été envoyé, mais les métadonnées n'étaient pas remplies correctement sur la feuille "Info". 
:::
...
::: {.instructions}
Ouvrez le fichier Excel `troubleshooting.xlsb` de l'archive que vous avez téléchargé en début de l'exercice.
:::
...
**Problème 3.** Il y a eu cinq patients décédés dans le centre, 3 notifiés en semaine 1 et 2 en semaine 2. Mais les chiffres sur la feuille d'analyse temporelle sont faux.
...
::: {.callout-note title="Indice" collapse=true}
...
::: {.callout-note title="Indice supplémentaires" collapse=true}
...
**Problème 3.** Peut être allez relire la @sec-customise au cas où il y a un paramètre à changer...
:::


::: {.callout-tip title="Réponse" collapse=true}
**Problème 1.** Des valeurs qui ne correspondent pas au menu déroulant ont été entré de force dans la colonne. Ces valeurs sont non valides et n'apparaissent pas dans le graphe ("Pos" au lieu de "Positif")


**Problème 2.** La variable "date de notification" n'a pas été vérifiée et nettoyée, et certaines valeurs ne sont même pas reconnues comme des dates par Excel. Les patients correspondant ne peuvent donc pas apparaître dans les analyses _temporelles_ parce vu que les formules sur la date vont planter. Par contre, le résumé général de la feuille univarié n'est pas conditionnel à la date (pas pour le nombre total de patients en tout cas), et n'est donc pas affecté par le problème.

**Problème 3.** Une des dates de notification était mal rentrée, et pas reconnue comme date. Une des valeurs du type de sortie avait une faute d'orthographe. Enfin, la date de début de semaine était paramétré pour le dimanche, alors que ce devrait être le lundi pour le Tchad. Regardez colonne calculée "semaine de notification", changez le jour de début de semaine (menu OBT) et voyez la se mettre à jour (type F9 pour recalculer si nécessaire).
:::
...
Pour vous connecter au tableau de bord, il vous faut un compte. La création de ce compte est semi-automatique si vous avez une adresse MSF : allez sur  [cette page](https://reports.msf.net/signup/).^[Si vous n'avez pas d'adresse MSF mais pensez que vous devriez avoir accès au tableau de bord, contactez l'équipe EpiDS.]

Une fois que vous avez un compte, connectez vous au [tableau de bord intersection](https://apps.epicentre-msf.org/secure/app/intersectional-surveillance) et utilisez votre compte crée pour vous connecter.


### Menu des filtres

Sur la gauche (numéro "1" sur @fig-dashboard) vous avez le menu des options, où vous pouvez filtrer les données (maladies, sections, pays, site, type de sortie etc.) et paramétrer quelques options.
...
Intéressons-nous d'abors à la carte.
...
::: {.callout-tip}
1. Il y a un bouton à gauche de la carte (numéro "3" on @fig-dashboard) qui permet de changer le fond de carte et de cacher / montrer les cercles ou les taux d'attaques pour améliorer la lisibilité

2. Si vous cliquez sur une région donnée, la courbé épidémique et la pyramide d'âge se mettront à jour pour ne montrer que les données des patients de cette région. La région sélectionnée est entourée d'un liseré rouge. Cliquez sur la région une seconde fois pour la désélectionner.
:::
...
![Apparence par défaut de la courbe épi](../module_1/images/013-img-epicurve.png){fig-align="center" #fig-dashboard}

::: {.instructions}
Utilisez les options du graphe (numéro 4 on @fig-dashboard) pour le modifier de telle sorte qu'il ressemble à ceci : 

![Hospitalisations au cours du temps](../module_1/images/014-img-epicurve-2.png){fig-align="center"}
:::

::: {.callout-tip}
1. La carte et la courbe épi ont un bouton "focus" en bas à droite, qui apparait quand on passe la souris dessus, et qui permet de passer la figure en plein écran ce qui est pratique si votre écran est petit (numéro 5 on @fig-dashboard).

2. Sur la courbe épi, vous pouvez faire glisser votre souris pour zoomer sur une période donnée. Un bouton "reset" permet de revenir à la vue par défaut.

3. Cliquez sur une barre du graphique pour mettre à jour la pyramide d'âge et la carte pour ne montrer que les cas de cette période. Un fond gris indique qu'une barre est sélectionnée. Cliquez une seconde fois sur la barre pour la désélectionner.

4. Cliquez sur une catégorie dans la légende pour la désélectionner et ne montrer que quelques catégories. Par exemple, sur le graphique ci-dessus vous pourriez décider de ne montrer que les patients hospitalisés en deux clics, ce qui est peut être plus rapide que d'ajuster les filtres globaux avec le menu latéral.
:::
...
- quand est ce que le tableau de bord a été mis à jour pour la dernière fois pour la rougeole ? (Indice : pas besoin de fouiller les sous-menus, c'est marqué à un endroit de la page principale)

- In 2024, quel pays a eu le plus de cas notifiés vis la liste linéaire ? Combien d'enfants de moins de six mois ont été notifiés ? Et en particulier, combien de filles ?
...
- Allez sur le site contenant la [Documentation]() and lisez comment [migrer les données et créer une liste linéaire "master"](https://epicentre-msf.github.io/outbreak-tools/how_to/migrate_linelist.html)