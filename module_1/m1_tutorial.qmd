...
::: {.callout-note}
This would happen before you receive a linelist with lots of data in it and explore it, but it was better to familiarise you with the file before attempting to troubleshoot it.
:::
...
Often people do not read the documentation, do not know the existence of the [OBT menu](../reference/ribbons.qmd#sec-linelist-ribbon) (or never checked it out). A lot of the requests you might get derive from this and the good news is that they are pretty straightforward to solve.
...
**Note**: recent versions of the linelist automatically add five lines at the bottom of the file when someone enters data in the last line, so this become less common. If the problem happens, check whether there is a more recent version of the linelist to use!
:::
...
- unsaved document was closed (data from "yesterday" or "this morning" were lost)
...
You can solve the last problem in one click though. Which button would you use?
...
A recent update^[We saw the problem hit computers between November and December 2024] in Excel 365 uses [strikethrough](https://support.microsoft.com/en-us/office/stale-value-formatting-4b5c63c3-5dc7-4e9b-8e24-88bed1987dbd) to label what Excel considers "stale values" (values that depend on other cells, which were modified). For some obscure reason, the "Refresh analyses" does not trigger the removal of the strikethrough. A fix in the OBT is coming from 2025 onwards, but if this happens with another linelist:
...
Alternately, you can [turn off](https://support.microsoft.com/en-us/office/stale-value-formatting-4b5c63c3-5dc7-4e9b-8e24-88bed1987dbd) this option if it bothers you in general.^[Maybe you use VBA in your own files and are bothered that Range.Calculate does _not_ remove the strikethrough, just Sheet.Caluclate or Application.Caclulate.]


### Sending the data to the dashboard

Another source of friction is the sharing step of the linelist. In all cases, it is good to brief them on the [steps](../how_to/export_data_dashboard.qmd) to send data to the dashboard, but see if you can find the exact problems with the files sent below.
...
**Problem 1.** They _know_ they have positive measles cases, but these cases do not show up in the univariate table / graph about Measles test results.
...
**Problem 3.** They know there should be 5 deceased patients in facilities, 3 who had been notified in week 1, and 2 notified in week 2. But the numbers in the temporal analysis sheet do not correspond.
