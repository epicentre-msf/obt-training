...
You can fill the variables either by using the cascading dropdown menus, or by using the [GeoHelper button](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html#sec-geo-helper) from the ribbon.
...
You are now going to work on the file `measles_linelist_moissala.xlsb`, which contains simulated measles data^[The data used in this exercise was simulated using parameters from MSF measles data, at least for some major variables, which avoids the problem of working with personnal data].
...
Another source of friction is the sharing step of the linelist. In all cases, it is good to brief colleagues using the linelist on the [steps](https://epicentre-msf.github.io/outbreak-tools/how_to/send_data_to_dashboard.html) to send data to the dashboard. For training, can you identify the source of these problems:
...
- A colleague sent `measles_linelist_moissala_export_data_20250203-1136.xlsx` to <EPI-DATA@epicentre.msf.org> and received an error
