...
downloadthis::download_link(
  link = "https://github.com/epicentre-msf/obt-training/blob/master/tutorials_materials/module_1.zip",
...
::: {.instructions}
Go to the [Geoapp](https://reports.msf.net/secure/app/outbreak-tools-geoapp) and download a geobase for Tchad. A summary of the steps can be found [here](https://epicentre-msf.github.io/outbreak-tools/how_to/import_geobase.html#sec-geobase-download), or in the popup at site opening.
:::
...
::: {.instructions}
Open the linelist (we removed the password for simplicity reasons) and  inspect sheet names.
:::
...
You are now going to work on the file `measles_linelist_moissala.xlsb`, which contains simulated measles data^[The data used in this exercise was simulated using parameters from MSF measles data, at least for some major variables, which avoids the problem of working with personnam data].
...
A lot of the requests you might receive derive from the fact that people ususally do not read the documentation (even if it was sent to them) and do not know the existence of the [OBT menu](https://epicentre-msf.github.io/outbreak-tools/reference/ribbons_linelist.html). The good news is that they are pretty straightforward to solve.
...
Another source of friction is the sharing step of the linelist. In all cases, it is good to brief them on the [steps](https://epicentre-msf.github.io/outbreak-tools/how_to/send_data_to_dashboard.html) to send data to the dashboard, but see if you can find the exact problems with these files:
